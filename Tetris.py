# Tetris Game in Python 3 / Windows 
...
def convert_shape_format(shape):
    ...
    for i, pos in enumerate(positions):
        positions[i] = (pos[0] - 2, pos[1] - 4) # 2 and 4 offset gets rid of the 'trailing dots' and sets the shape in the correct position on the screen
        ...
        if (0,0,0) not in row:
            ...
            for j in range(len(row)):
                try:
                    ...
                    continue
    # shifting top rows down (or actually adding a new row on top)
    if increment > 0:
